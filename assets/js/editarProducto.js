document.querySelectorAll(".btnEditarProducto").forEach((editarProducto) => {
  editarProducto.addEventListener("click", async function () {
    let idProducto = this.dataset.id;

      try {
        //obtener el producto usando ajax
        const obtenerProducto = await fetch(
          `/cafeElBuenSabor/functions/obtenerProducto.php`,
          {
            method: "POST",
            headers: {
        ...
        document.querySelector("#editarNombre").value =
          resObtenerProducto.nombre;
        document.querySelector("#editarDescripcion").value =
          resObtenerProducto.descripcion;
        document.querySelector("#editarPrecio").value =
          resObtenerProducto.precio;
